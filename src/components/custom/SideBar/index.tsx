		...
		<Sider width="220" trigger={null} collapsible collapsed={state.collapsed}>
			<div className="relative flex flex-col h-full w-full bg-background">
				<Button
					...
					type="text"
					icon={
						state.collapsed ? <ArrowRightOutlined /> : <ArrowLeftOutlined />
					}
				...
				>
					<div
						className={cn("flex items-center", state.collapsed && "flex-col")}
					>
						{!state.collapsed ? (
					...
					{!state.collapsed ? (
						<Popconfirm
							title="确定要退出登录吗?"
							onConfirm={globalModel.logout}
						>
							<Button type="text">
								<span className="mr-2">退出</span>
								<LogoutOutlined />
							</Button>
						</Popconfirm>
					) : (
